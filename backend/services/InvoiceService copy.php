...
//$startTime is defined outside the function
// Closures don’t automatically access outer variables
// We need to measure time from $startTime to now
// We must "import" $startTime into the closure
...
//calculate the newCents value,that is the sum of currentCents and itemCents with the product of the item quantity
...
Else → build structured output.*/
    ...
    // ===================================================================
    // Sort items by price in descending order (higher-priced items first)
usort($items, fn($a, $b) => $b['price'] <=> $a['price']);
